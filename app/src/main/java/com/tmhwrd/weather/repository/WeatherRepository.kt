...
class WeatherRepository(private val database: ForecastDatabase) {
    val forecasts: LiveData<List<Forecast>> = database.forecastDao.getAll()
    ...
    suspend fun fetchForecasts() {
        ...
        withContext(Dispatchers.IO) {
            database.forecastDao.insertAll(forecasts.filterNotNull().domainObjects)
        }
    }
    ...
    private suspend fun fetchForecast(capital: Pair<String, Int>): WeatherDTO? {
        ...
        return fiveDay?.let {
            WeatherDTO(
                capital.first, Date(), currentConditions?.firstOrNull() ?: CurrentConditions(), it
            )
        }
    }
